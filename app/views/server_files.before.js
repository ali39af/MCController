...
module.exports.load = function (callback) {
    fs.readFile(`./themes/${app.ysql.getData('app_theme')}/server_files.html`, 'utf8', function (err, data) {
        if (!err) {
            html_files = data;
            fs.readFile(`./themes/${app.ysql.getData('app_theme')}/server_components.html`, 'utf8', function (err, data) {
                if (!err) {
                    ...
                    fs.readFile(`./themes/${app.ysql.getData('app_theme')}/403.html`, 'utf8', function (err, data) {
                        if (!err) {
                            html_error_403 = data;
                            app.app.get('/server/files/', function (req, res) {
                            ...
                            app.app.post('/server/files/', function (req, res) {
                                if (app.ysql.getData('app_connection_type') == 'private') {
                                    if (req.ipInfo.ip != '::1') {
                                        return;
                                    }
                                ...
                                if (app.ysql.getData('app_blacklist_ip').split('\n').includes(req.ipInfo.ip) == true) {
                                    ...
                                    app.user_login(req.body.token, function (user) {
                                        if (user.status != 'error') {
                                            if (user.level_permission.split(',').includes('all') == true || user.level_permission.split(',').includes('files') == true) {
                                                app.nav_side_list.list(user.level_permission.split(','), 'files', function (nav_side_list) {
                                                    let new_html_files = html_files.replaceAll('${server_components}', html_server_components);
                                                    new_html_files = new_html_files.replaceAll('${nav_side_list}', nav_side_list);
                                                    res.send(app.app_languages(new_html_files, app.ysql.getData('app_language')));
                                                });
                                            } else {
                                                res.redirect('/server/access/');
                                            }
                                        } else {
                                            if (req.ipInfo.ip != '::1') {
                            ...
                            app.loger.log('Loading View Files Done!', 1, 1);
                            callback('true');
                        } else {
                            app.loger.log('Loading View Files.403 Failed!', 3, 3);
                        }
                    });
                } else {
                    app.loger.log('Loading View Files Components Failed!', 3, 3);
                }
            });
        } else {
            app.loger.log('Loading View Files Failed!', 3, 3);
        }
    });
}